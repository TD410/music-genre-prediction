...
FEATURE_MODEL_BATCH_SIZE = 300

class GenresInputGenerator(Sequence):
    def __init__(self, path, batch_size = FEATURE_MODEL_BATCH_SIZE):
        self.path = path
    ...
    def __getitem__(self, batch_index):
        batch_x = np.empty((self.batch_size, 13))
...
def train(model):
    train_generator = GenresInputGenerator(FEATURE_NORM_DIR + 'train/')
    test_generator = GenresInputGenerator(FEATURE_NORM_DIR + 'test/')

    checkpoint = ModelCheckpoint('Models/featureModel.h5', save_weights_only=False, save_best_only=True, period=1, verbose=1)
    reduce_lr = ReduceLROnPlateau(monitor='loss', factor=0.1, patience=20, verbose=1)
    early_stopping = EarlyStopping(monitor='val_loss', min_delta=0.001, patience=100, verbose=1, mode='auto')
    ...
    with open('Logs/featureModelHistory', 'wb') as file:
        pickle.dump(history.history, file)

def test(model):
    train_generator = GenresInputGenerator(FEATURE_DIR+ 'train/')
    test_generator = GenresInputGenerator(FEATURE_DIR + 'test/')
...
def draw_history():
    history = pickle.load(open('Logs/featureModelHistory', 'rb'))
